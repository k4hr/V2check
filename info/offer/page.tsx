  ...
  return (
    <main className="lm-page" style={{ maxWidth: 900 }}>
      ...
      <p>
        ...
        соответствии со статьёй 437 Гражданского кодекса РФ и определяет условия
        предоставления доступа к функционалу сервиса LiveManager (далее —
        «Сервис», «Исполнитель») пользователям (далее — «Пользователь»).
      </p>
      ...
      <ul>
        <li>
          ...
          провайдерами, платформами (Telegram/VK), провайдерами платежей,
          форс-мажором.
        </li>
        <li>
      ...
      <p>
        <i>Замените плейсхолдеры на реальные данные перед публикацией.</i>
      </p>
      <ul>
      ...
      <p>
        Цены автоматически подтягиваются из внутреннего файла{' '}
        <code>lib/pricing.ts</code>. Для <b>Telegram</b> стоимость указана в{' '}
        <b>XTR (Stars)</b>. Для <b>VK</b> — в <b>рублях</b> (итоговые суммы
        формируются из копеек). Конечная сумма, отображаемая платёжной
        площадкой, может отличаться из-за комиссий/налогов площадки и является
        приоритетной.
      </p>
      ...
      <p style={{ marginTop: 24 }}>
        Актуальная редакция Оферты доступна по адресу:{' '}
        <Link href="/legal/offer">/legal/offer</Link>
      </p>
    </main>
  );
}
