...
export default function HomePage(){
  ...
  useEffect(()=>{
    const w:any=window; try{ w?.Telegram?.WebApp?.ready?.(); w?.Telegram?.WebApp?.expand?.(); }catch{}
  ...
  const linkSuffix=useMemo(()=>{
    try{ const u=new URL(window.location.href); const sp=new URLSearchParams(u.search);
      sp.set('welcomed','1'); const id=u.searchParams.get('id'); if(id) sp.set('id',id);
      const s=sp.toString(); return s?`?${s}`:''; }catch{ return '?welcomed=1'; }
  },[]);
  const href=(p:string)=>`${p}${linkSuffix}` as Route;

  async function onSave(){
    if(saving) return; setSaving(true); setLocaleEverywhere(pendingLocale);
    const url=new URL(window.location.href); url.searchParams.set('_lng',String(Date.now()));
  ...
  return (
    <main>
      ...
      <div className="lm-grid" style={{marginTop:16}}>
        {/* стеклянно-золотая карточка */}
        <Link href={href('/home/ChatGPT')} className="card hero-card" style={{textDecoration:'none'}}>
          <span style={{flex:1,display:'flex',alignItems:'center',justifyContent:'center'}}>
            <span className="card__title gold-text" style={{fontSize:18}}>CHATGPT 5</span>
          </span>
          <span className="card__chev">›</span>
        </Link>

        <Link href={href('/cabinet')} className="card" style={{textDecoration:'none'}}>
          <span className="card__left"><span className="card__title">{L.cabinet}</span></span>
          <span className="card__chev">›</span>
        </Link>

        <Link href={href('/pro')} className="card card--pro" style={{textDecoration:'none'}}>
          <span className="card__left"><span className="card__title">{L.buy} <span className="badge">{L.pro} / {L.proplus}</span></span></span>
          <span className="card__chev">›</span>
        </Link>

        <Link href={href('/home/pro')} className="card card--pro" style={{textDecoration:'none'}}>
          <span className="card__left"><span className="card__title">{L.daily} <span className="badge">{L.pro}</span></span></span>
          <span className="card__chev">›</span>
        </Link>

        <Link href={href('/home/pro-plus')} className="card card--proplus" style={{textDecoration:'none'}}>
          <span className="card__left"><span className="card__title">{L.expert} <span className="badge badge--gold">{L.proplus}</span></span></span>
          <span className="card__chev">›</span>
        </Link>
      </div>
      ...
      {platform!=='vk' && (
        <>
          <div style={{marginTop:18,display:'flex',justifyContent:'center'}}>
            <button type="button" onClick={()=>setOpen(v=>!v)} className="ghost-link" style={{textDecoration:'none'}}>
              {L.changeLang}
            </button>
          </div>

          {open && (
            <div style={{marginTop:12,border:'1px dashed rgba(45,126,247,.25)',background:'rgba(255,255,255,.62)',backdropFilter:'blur(10px)',borderRadius:14,padding:14,maxWidth:560,marginLeft:'auto',marginRight:'auto'}}>
              ...
              <div style={{display:'flex',gap:10,justifyContent:'flex-end',marginTop:12}}>
                ...
                <button type="button" onClick={onSave} disabled={saving||pendingLocale===currentLocale}
                  className="list-btn"
                  style={{padding:'10px 14px',borderRadius:12,background:'linear-gradient(180deg, rgba(45,126,247,.12), rgba(45,126,247,.08))',border:'1px solid #4b57b3',opacity:saving?0.7:1}}>
      ...
      <NewsSection locale={currentLocale==='en'?'en':'ru'} items={visibleNews} />
    </main>
  );
}
