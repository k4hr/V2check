  ...
  return (
    <main className="home">
      ...
      <section className="bento">
        ...
        <Link href={href('/home/ChatGPT')} legacyBehavior>
          <a className="tile tile--hero glass">
            <span className="tile__content"><span className="hero__label">CHATGPT 5</span></span>
            <span className="chev">›</span>
            <span className="hero__glow" aria-hidden />
          </a>
        </Link>

        <Link href={href('/cabinet')} legacyBehavior>
          <a className="tile glass">
            <span className="tile__content"><span className="tile__title">{L.cabinet}</span></span>
            <span className="chev">›</span>
          </a>
        </Link>

        <Link href={href('/pro')} legacyBehavior>
          <a className="tile glass">
        ...
        {platform!=='vk' && (
          <button type="button" onClick={()=>setOpenLang(v=>!v)} className="tile glass as-button">
            <span className="tile__content"><span className="tile__title">{L.changeLang}</span></span>
            <span className="chev">›</span>
          </button>
        )}
      </section>
        ...
        <div className="lang-sheet glass">
          ...
          <div className="lang-grid">
            {LOCALES.map(l=>{
              ...
              return (
                <button key={l.code} onClick={()=>setPendingLocale(l.code)}
      ...
      <style jsx>{`
        ...
        /* Ссылка-как-кнопка: убираем синий/подчёркнутый */
        .tile:link, .tile:visited { color:#0d1220; text-decoration:none; }
        ...
        .badge{ margin-left:8px; padding:2px 8px; border-radius:999px; font-size:12px; font-weight:700;
        ...
        .tile--hero{ grid-column:1 / -1; min-height:86px; overflow:hidden; isolation:isolate; }
        .hero__label{
        ...
        .tile--gold{ box-shadow:0 12px 32px rgba(231,186,82,.12), inset 0 0 0 1px rgba(255,255,255,.55); border-color:rgba(231,186,82,.45); }

        .lang-sheet{ margin:16px auto 0; padding:14px; border-radius:16px; max-width:560px; }
