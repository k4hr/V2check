...
export default function CinemaConcierge() {
  ...
  useEffect(() => { listRef.current?.scrollTo({ top: 999999, behavior: 'smooth' }); }, [messages, loading]);

  const idSuffix = useMemo(() => {
    try { const u=new URL(window.location.href); const id=u.searchParams.get('id'); return id?`?id=${encodeURIComponent(id)}`:''; }
    catch { return ''; }
  }, []);
  ...
  async function send() {
    ...
    setLoading(true);
    try {
      // универсальный /api/ai (как в проекте). Модель для Pro — 4o-mini.
  ...
  return (
    <main style={{padding:20, maxWidth:800, margin:'0 auto', display:'grid', gap:12}}>
      ...
      <div
        ref={listRef}
        style={{
          border:'1px solid var(--border)', borderRadius:14, background:'#121722',
          padding:12, height:'54vh', overflow:'auto'
        }}
      >
        {messages.filter(m=>m.role!=='system').map((m, i) => (
          <div key={i} style={{margin:'10px 0', display:'flex', justifyContent: m.role==='user'?'flex-end':'flex-start'}}>
            <div style={{
              maxWidth:'82%', padding:'10px 12px', borderRadius:12, lineHeight:1.5,
              background: m.role==='user' ? '#24304a' : '#1a2132',
              border: '1px solid #2b3552', whiteSpace:'pre-wrap'
            }}>
              {m.content}
            </div>
          </div>
      ...
      <div style={{display:'flex', gap:8}}>
        <input
          ...
          style={{
            flex:1, padding:'12px 12px', borderRadius:12, border:'1px solid var(--border)',
            background:'#121722', color:'var(--fg)'
          }}
        />
