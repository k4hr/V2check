  ...
  return (
    <main className="lm-page" style={{ maxWidth: 900 }}>
      ...
      <ul>
        ...
        <li>
          Оплата принимается через платёжные провайдеры (в т.ч. YooKassa/ЮKassa,
          Robokassa, VK&nbsp;Pay и иные, указанные в интерфейсе). Платёж считается
          совершённым после подтверждения провайдера. Данные банковских карт
          обрабатываются и хранятся исключительно провайдерами; Исполнитель не
          хранит реквизиты карт.
        </li>
        <li>
          ...
          отключить до даты списания в интерфейсе провайдера/Сервиса либо
          через поддержку Исполнителя (см. п. 5.4).
        </li>
      </ul>
      ...
      <ul>
        <li>
          ...
          оплаты у платёжного провайдера и на последующие рекуррентные списания
          стоимости месячной Подписки по истечении 24 часов с момента оплаты
          пробного периода и далее с периодичностью 1 месяц до отмены
          Пользователем.
        </li>
        <li>
          Сумма последующего списания равна действующей цене месячного тарифа на
          момент продления (см. Приложение&nbsp;1). Провайдер может выполнять
          техническую проверку карты (предавторизацию) на небольшую сумму.
        </li>
        <li>
          Отменить автопродление можно в любое время до даты следующего
          списания: (а) в интерфейсе провайдера, (б) в интерфейсе Сервиса
          (когда доступно), либо (в) через поддержку Исполнителя — отправив
          запрос на <b>support@seimngr</b> или в чат поддержки с указанием ID
          аккаунта/Telegram&nbsp;ID и фразой «Отмена подписки». Отмена
          обрабатывается в течение 24 часов (в рабочее время). После отмены
          доступ сохраняется до конца уже оплаченного периода; перерасчёт/
          возврат за неиспользованный остаток периода не производится, за
          исключением случаев, указанных в разделе 6.
        </li>
        <li>
          Чеки по рекуррентным списаниям формируются и направляются провайдером
          на указанный Пользователем e-mail.
        </li>
      </ul>

      <h3>5.2. Согласие на оффлайн-списания</h3>
      <p>
        Оформляя Подписку/пробный период, Пользователь предоставляет
        Исполнителю и платёжному провайдеру право инициировать повторные
        списания без дополнительного подтверждения (off-session) с ранее
        сохранённого способа оплаты до момента отмены Подписки. Согласие может
        быть отозвано путём отмены автопродления согласно п.&nbsp;5.1.
      </p>
      ...
      <ul>
        ...
        <li>Поддержка для отмены/претензий: <b>support@seimngr</b></li>
