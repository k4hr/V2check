...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!tkey) return NextResponse.json({ ok:false, error:'KEY_REQUIRED' }, { status:400 });

    const user = await prisma.user.findUnique({ where: { telegramId: tkey } });
    ...
    const updated = await prisma.user.update({
      ...
      data: {
        autopayActive: false,
        autopayNextAt: null,
        // опционально: trialActive: false
        // ykPaymentMethodId не трогаем (чтобы видеть привязку), но можно обнулить при желании.
      },
      select: { id: true, telegramId: true, autopayActive: true, autopayNextAt: true },
    });
