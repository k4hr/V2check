...
export async function POST(req: Request) {
  try {
    ...
    // Цена: обычная или пробная (1 ₽)
    const baseK  = getVkRubKopecks(tier)[plan];
    const finalK = trial ? 100 : applyDiscountKopecks(plan, baseK);

    const shopId = process.env.YK_SHOP_ID!;
    const secret = process.env.YK_SECRET_KEY!;
    const envBase = (process.env.YK_RETURN_URL_BASE || process.env.APP_ORIGIN || process.env.NEXT_PUBLIC_APP_ORIGIN || '').replace(/\/+$/, '');
    const returnBase = envBase || getOrigin(req);
    if (!shopId || !secret || !returnBase) {
      return NextResponse.json({ ok: false, error: 'YKassa env not set (SHOP_ID/SECRET/RETURN_BASE)' }, { status: 500 });
    }
    ...
    const idempotenceKey = sha256(`${ip}:${ua}:${tier}:${plan}:${trial ? 'TRIAL' : 'STD'}:${minuteBucket}`).slice(0, 48);

    const description = trial
      ? `LiveManager ${tier} — Пробный день (далее ${plan})`
      : `LiveManager ${tier} — ${plan}`;
    ...
    const metadata: Record<string, any> = {
      ...
      ...(email && { email }), // <— сохраняем email в metadata для автосписания
    };
    ...
    const payload: any = {
      amount: { value: fmt(finalK), currency: 'RUB' },
      capture: true,
      save_payment_method: trial ? true : undefined, // для триала сохраняем метод
