...
export async function POST(req: NextRequest) {
  try {
    ...
    // Попробуем определить telegramId из query (?id=...) как это делалось в /api/me
    ...
    // Бесплатный лимит для не-Pro: по cookie
    if (!isPro) {
      ...
      // разрешаем запрос, но увеличим счетчик в ответе (чтобы браузер поставил cookie)
      ...
      const resp = NextResponse.json({ ok: true, answer, pro: false, freeLimit: FREE_QA_PER_DAY, used: used + 1 });

      // ставим cookies на сутки (UTC)
    ...
    return NextResponse.json({ ok: true, answer, pro: true });
  } catch (e: any) {
